...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const userCommentIdsFromUsers = database.users[savedComment.username].commentIds;
    userCommentIdsFromUsers.splice(userCommentIdsFromUsers.indexOf(id), 1);

    const userCommentIdsFromArticles = database.articles[savedComment.articleId].commentIds;
    userCommentIdsFromArticles.splice(userCommentIdsFromArticles.indexOf(id), 1);

    response.status = 204;
  } else if(!id || database.comments[id] == null){
    response.status = 404;
  } else
  {
    response.status = 400;
...
//Source => https://www.npmjs.com/package/write-yaml
function saveDatabase(){
  yaml(databaseFile, database, function(err) {
    if (err) throw err;
    console.log(database);
  });
}

//Source => https://www.npmjs.com/package/read-yaml
function loadDatabase(){
  readYaml(databaseFile, function(err, database) {
    if (err) throw err;
    console.log(database);
  });
}
